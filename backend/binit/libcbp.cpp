/* modified sample so as to enable bootstraping */
#include <iostream>
#include <ostream>
#include <ctime>
#include <vector>
...
namespace CBuildP {
    ...
    CBuildP::file_t _outfile;
    CBuildP::file_t _link;
    CBuildP::file_t _flags;
    ...
    std::string get_files(int argc, char *argv[]) {
        ...
        if (!is_file_C_CXX(std::string(argv[1 + __offset]))) {
            std::cout << "[ERROR] : Non C/C++ file included, are you sure?\n"
    ...
    void optimize(const opt_t &opt) {
        ...
        std::cout << "\n" << hex_to_ansi(COLOR_INFO, &_arn, false).pointer << ":: " << RESET << hex_to_ansi(COLOR_SUCCESS, &_arn, false).pointer << "DONE" << std::endl;
    }

    void specs_dest() {
        std::cout << hex_to_ansi(COLOR_MUTED, &_arn, false).pointer << "Build output to " << RESET << _outfile << std::endl;
    }
    ...
    void compile(const file_t &file_names, const file_t & outfile) {
        std::cout << hex_to_ansi(COLOR_INFO, &_arn, false).pointer << ":: " << RESET << "Compiling: " << hex_to_ansi(COLOR_MUTED, &_arn, false).pointer << file_names << RESET << std::endl;
        std::string command = std::string(_stdopt.compiler) + " " + "-O" + std::to_string(_stdopt.level) + " " + file_names + " -o " + outfile;
        int result = system(command.c_str());
        if (result != 0) {
...
int main(int argc, char *argv[]) { str_initArena(&_arn, NULL); conf_build_cmd(argc, argv); build_main(argc, argv); std::cout << hex_to_ansi(_code == 0 ? COLOR_SUCCESS : COLOR_ERROR, &_arn, false).pointer << " CODE " << _code << RESET <<  std::endl; conf_build_end(); CBuildP::specs_dest(); return _code; }
